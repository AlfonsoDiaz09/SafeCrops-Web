...
// (Modelo Transformer) Importaciones de funciones en otros archivos JS
...
const data_formulario = document.querySelector('[data-formulario-transformer]');
...
inputs.forEach((input) => {
    console.log(input)
    input.addEventListener('keyup', (nombre=='ModeloTransformer') ? validarFormularioModeloTransformer : ''); // Cuando se levanta la tecla
    input.addEventListener('blur', (nombre=='ModeloTransformer') ? validarFormularioModeloTransformer : ''); // Cuando sale del input
    input.addEventListener('change', (nombre=='ModeloTransformer') ? validarFormularioModeloTransformer : ''); // Cuando sale del input
    input.addEventListener('input', (nombre=='ModeloTransformer') ? validarFormularioModeloTransformer : ''); // Cuando sale del input
});


// Agregar eventos a cada select para ejecutar una función
selects.forEach((select) => {
    select.addEventListener('click', (nombre=='ModeloTransformer') ? validarFormularioModeloTransformer : ''); // Cuando da un click dentro del select
    select.addEventListener('change', (nombre=='ModeloTransformer') ? validarFormularioModeloTransformer : ''); // Cuando se hace un cambio en la opción
    select.addEventListener('blur', (nombre=='ModeloTransformer') ? validarFormularioModeloTransformer : ''); // Cuando sale del select
});

formulario.addEventListener('submit', (e) => {
    switch (accionNombre){
        case 'Crear-ModeloTransformer':
